    ...
    def register(self):
        ...
        if response.headers.get("WWW-Authenticate"):
            ...
            if realm_match and nonce_match:
                ...
                response = generate_digest_challenge_response(self.username, self.password, realm, nonce, "REGISTER", uri)
                headers["Authorization"] = f'Digest username="{self.username}", realm="{realm}", nonce="{nonce}", uri="{uri}", response="{response}", algorithm=MD5'
                message = SipMessage(method="REGISTER", uri=uri, headers=headers)
                self.transport.send(message.to_string(), (self.server, self.port))
                self.cseq += 1
