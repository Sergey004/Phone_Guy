...
class RTPClient:
    def __init__(
        ...
        self.NSD = True
        self.assoc = {payload_type: PayloadType(payload_type)}
        self.preference = None
        for pt in self.assoc:
            try:
                if int(self.assoc[pt]) is not None:
                    self.preference = self.assoc[pt]
                    break
            except Exception:
                pass

        if self.preference is None:
            self.preference = PayloadType.PCMA
        ...
        self.pmout = RTPPacketManager()
        self.pmin = RTPPacketManager()
        ...
        self.trans_delay_reduction = 1.0
        self.sin = None
        self.sout = None
    ...
    def trans(self) -> None:
        while self.NSD:
            ...
            self.outSequence += 1
            self.outTimestamp += 160  # Fixed 160 bytes per frame for G.711
    ...
    def encode_packet(self, payload: bytes) -> bytes:
        # If payload is already G.711 (160 bytes), pass through directly
        if len(payload) == 160:
            return payload
    ...
    def local_addr(self) -> Tuple[str, int]:
        return (self.inIP, self.inPort)

    @property
    def remote_addr(self) -> Tuple[str, int]:
        return (self.outIP, self.outPort)

    def set_remote(self, addr: Tuple[str, int]) -> None:
        self.outIP, self.outPort = addr


def create_rtp_client(
    local_ip: str,
    local_port: int,
    remote_ip: str,
    remote_port: int,
